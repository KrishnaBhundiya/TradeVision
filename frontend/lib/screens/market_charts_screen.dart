...
class MarketChartsScreen extends StatefulWidget {
  ...
  @override
  State<MarketChartsScreen> createState() => _MarketChartsScreenState();
}

class _MarketChartsScreenState extends State<MarketChartsScreen> {
  ...
  List<FlSpot> get _activeChartData {
    switch (_selectedTimeframe) {
      case '1W':
        return _selectedStock.chart1W;
      case '1M':
        return _selectedStock.chart1M;
      case '1Y':
      case '3M':
      case '6M':
      case '5Y':
      case 'MAX':
        return _selectedStock.chart1Y;
      case '1D':
      default:
        return _selectedStock.chart1D;
    }
  }

  List<OhlcPoint> get _activeOhlcData {
    final now = DateTime.now();
    final spots = _activeChartData;
    ...
    return List.generate(spots.length, (i) {
      ...
      final time = now.subtract(
        Duration(days: (spots.length - 1 - i) * (_selectedTimeframe == '1D' ? 1 : 5)),
      );
      return OhlcPoint(
        time: time,
        open: open,
            ...
            child: Column(
              ...
              children: [
                ...
                _buildChartTip(
                  ...
                  text: 'Green line going up means the stock price is rising -- good sign',
                ),
                _buildChartTip(
                  icon: Icons.trending_down_rounded,
                  color: const Color(0xFFFF3B3B),
                  text: 'Red line going down means the stock price is falling',
                ),
                _buildChartTip(
                  icon: Icons.bar_chart_rounded,
                  color: const Color(0xFF0066CC),
                  text: 'Tap 1D, 1W, 1M to see different time periods',
                ),
              ],
            ),
          ),
      ...
      child: Column(
        ...
        children: [
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      Text(
                        _selectedStock.changeAmountFormatted,
                        style: TextStyle(
                          fontSize: 14,
                          fontWeight: FontWeight.w700,
              ...
              Row(
                children: [
                  IconButton(
                    icon: Icon(
                      _isCandleMode ? Icons.show_chart : Icons.candlestick_chart,
                      color: theme.colorScheme.primary,
                    ),
                    onPressed: () => setState(() => _isCandleMode = !_isCandleMode),
                  ),
                  Row(
                    ...
                    children: [
                      ...
                      GestureDetector(
                        onTap: () {
                          ...
                          _showSignalExplanation(context, _selectedStock.aiSignal);
                        },
          ...
          InteractiveStockChart(
            rawData: _activeOhlcData,
            chartType: _selectedChartType,
            period: _selectedTimeframe,
            currentPrice: _selectedStock.price,
          ),
  ...
  Widget _buildIndicatorRow(String name, String val, String sub) {
    ...
    return Row(
      mainAxisAlignment: MainAxisAlignment.spaceBetween,
      children: [
        Text(name, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w700, color: theme.colorScheme.onSurface)),
        Column(
          crossAxisAlignment: CrossAxisAlignment.end,
          children: [
            Text(val, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w800, color: theme.colorScheme.onSurface)),
            Text(sub, style: TextStyle(fontSize: 11, color: theme.colorScheme.onSurface.withOpacity(0.65), fontWeight: FontWeight.w500)),
          ],
        ),
      ],
    );
